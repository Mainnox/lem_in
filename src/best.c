...
/*   Updated: 2019/09/12 15:53:55 by akremer          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int		setup_best(t_info *handle)
{
	if (!(handle->best = (t_best*)malloc(sizeof(t_best))))
		return (1);
	if (!(handle->best->banlist = (int*)malloc(sizeof(int) * handle->graph->nb_vertices * 10)))
		return (1);
	if (!(handle->best->best = (int*)malloc(sizeof(int) * handle->graph->tab_neigh[1].nb_path * 10)))
		return (1);
	if (!(handle->best->tmp_best = (int*)malloc(sizeof(int) * handle->graph->tab_neigh[1].nb_path * 10)))
		return (1);
